	...
	case shimSet:
	  ...
	  break;
        ...
        case RFMonVal:
          ...
          break;
	case mpsDNP:
	  strncpy(txt,"mpsDNP",TXT_LEN);
	  break;
	case mpsEPR:
	  strncpy(txt,"mpsEPR",TXT_LEN);
	  break;
	case mpsRfOff:
	  strncpy(txt,"mpsRfOff",TXT_LEN);
	  break;
	case mpsRfOn:
	  strncpy(txt,"mpsRfOn",TXT_LEN);
	  break;
	case mpsRfExt:
	  strncpy(txt,"mpsRfExt",TXT_LEN);
	  break;
	case mpsPower:
	  strncpy(txt,"mpsPower",TXT_LEN);
	  break;
	case mpsFreq:
	  strncpy(txt,"mpsFreq",TXT_LEN);
	  break;
	case mpsRxMv:
	  strncpy(txt,"mpsRxMv",TXT_LEN);
	  break;
	case mpsLkStat:
	  strncpy(txt,"mpsLkStat",TXT_LEN);
	  break;
	case mpsTemp:
	  strncpy(txt,"mpsTemp",TXT_LEN);
	  break;
	case mpsAmpStat:
	  strncpy(txt,"mpsAmpStat",TXT_LEN);
	  break;
	case mpsTxMv:
	  strncpy(txt,"mpsTxMv",TXT_LEN);
	  break;
 ...
 *  power_format() call */
    ...
    if (CurrentStatBlock.AcqSpinSet != statblock->AcqSpinSet)
    {
        ...
        if (CurrentStatBlock.AcqSpinSet >= 0)
        {
            ...
            sprintf(buf,"%7.6f", (double) CurrentStatBlock.AcqSpinSet * 1e-6);
        }
        else
        {
            if (SPNpan2)  {
    ...
    if (CurrentStatBlock.AcqSpinAct != statblock->AcqSpinAct)
    {
        ...
        if (CurrentStatBlock.AcqSpinAct >= 0) {
            ...
            sprintf(buf,"%4.1f", (double) CurrentStatBlock.AcqSpinAct / 10.0);
        }
        else
            buf[0]=0;
    ...
    if (CurrentStatBlock.AcqSpinSpeedLimit != statblock->AcqSpinSpeedLimit)
    {
        ...
        if (CurrentStatBlock.AcqSpinSpeedLimit >= 0)
            sprintf(buf,"%4.1f", (double) CurrentStatBlock.AcqSpinSpeedLimit / 10.0);
        else
            buf[0]=0;
        disp_string(mpsTxMv,buf);
    }
    ...
    if (CurrentStatBlock.AcqSpinAdj != statblock->AcqSpinAdj)
    {
        ...
        if (CurrentStatBlock.AcqSpinAdj == 0)
        {
            disp_string(mpsDNP,"0");
            disp_string(mpsEPR,"1");
            disp_string(mpsRfOff,"1");
            disp_string(mpsRfOn,"0");
            disp_string(mpsRfExt,"0");
	}
        ...
        {
            disp_string(mpsDNP,"1");
            disp_string(mpsEPR,"0");
	}
    }
    ...
    if (CurrentStatBlock.AcqSpinProfile != statblock->AcqSpinProfile)
    {
        ...
        if (CurrentStatBlock.AcqSpinProfile >= 0)
            sprintf(buf,"%4.1f", (double) CurrentStatBlock.AcqSpinProfile / 10.0);
        else
            buf[0]='\0';
        disp_string(mpsPower,buf);
    }
