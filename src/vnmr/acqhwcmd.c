...
/*  The replacement programs are here ... */
	...
	else if ( (hw_arg = find_arg(argc,argv,"mps") ) != 0)
        {
           if (hw_arg+1 < argc)
           {
              if ((int) strlen(argv[hw_arg+1]) > msglen - 20)
              {
                 Werrprintf("sethw('mps') command '%s' is too long", argv[hw_arg+1] );
	         return( -1 );
              }
              if (sethw_wait)
              {
                 ...
                 sprintf(msgptr,"%s/sethw", curexpdir);
                 unlink(msgptr);
                 sprintf(msgptr,"outfile %s/sethw %s",
                         curexpdir,argv[hw_arg+1]);
              }
              else
              {
                 sprintf(msgptr,"%s",argv[hw_arg+1]);
              }
	      return( 0 );
           }
           else
           {
              Werrprintf("sethw('mps') requires a second argument");
	      return( -1 );
           }
        }
        else
	...
	if (stat != FAILED)
        {
          if ( find_arg(argc,argv,"robot") )
             cmd = ROBOT;
	  else if ( find_arg(argc,argv,"mps") )
             cmd = MPS;
          else
             cmd = ACQHARDWARE;
...
/*  At this time, the VNMR program only checks the new values once.
	...
	if ( ((cmd == ROBOT) || (cmd == MPS)) && sethw_wait)
        {
           char path[MAXPATH];
           char msg[MAXPATH];
